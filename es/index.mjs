...
function createFileSchema(bucket) {
  ...
  FileSchema.methods.read = function read(optns, done) {
    ...
    const { _id } = this;
    const { filename } = this;
    const $$optns = merge$1({}, $optns, { _id, filename });
...
GridFSBucket.prototype.createWriteStream = function createWriteStream(optns) {
  ...
  const options = merge$1({}, defaults, optns);
...
GridFSBucket.prototype.writeFile = function writeFile(file, readstream, done) {
  // ensure file details
  const $file = merge$1({}, { _id: new ObjectId() }, file);
...
function createBucket(optns = {}) {
  ...
  const options = merge$1({}, DEFAULT_BUCKET_OPTIONS, omit$1(optns, 'connection'));
...
function createModel(optns, ...plugins) {
  // ensure options
  let { connection, modelName, bucketName } = merge$1({}, optns);
  connection = connection || mongoose.connection;
  ...
  // create bucket
  const options = merge$1({}, { connection, modelName, bucketName }, optns);
  ...
  const schemaOptions = merge$1({}, schema.options);
